...
use nom::{alphanumeric, not_line_ending, space, ErrorKind};
...
named!(
    octal_digit<u8>,
    map!(one_of!("01234567"), |b| b as u8 - b'0')
);
...
named!(
    ...
    delimited!(
        tag!("\""),
        map_res!(
            many0!(alt!(map!(none_of!("\\\""), |b| b as u8) | hex_esc)),
            String::from_utf8
        ),
        tag!("\"")
    )
...
mod tests {
    use super::{
        attribute, comment, endl, hex_digit, hex_esc, multiline_octal, octal_digit, octal_esc,
        quad_digit, quoted_string, token, type_and_value, Value,
    };
    use nom::IResult::*;
    ...
    fn test_comment() {
        ...
        assert_eq!(
            comment(b"bees"),
            Error(Err::Position(ErrorKind::Tag, &b"bees"[..]))
        );
        assert_eq!(
            comment(b" #bees"),
            Error(Err::Position(ErrorKind::Tag, &b" #bees"[..]))
    ...
    fn test_endl() {
        ...
        assert_eq!(
            endl(b"# bees"),
            Incomplete(Needed::Size("# bees".len() + 1))
        );
    }
    ...
    fn test_token() {
        ...
        assert_eq!(
            token(b"CKA_CERT_MD5_HASH"),
            Done(&b""[..], "CKA_CERT_MD5_HASH".to_owned())
        );
        assert_eq!(token(b"UTF8 "), Done(&b" "[..], "UTF8".to_owned()));
        assert_eq!(token(b"UTF8\n"), Done(&b"\n"[..], "UTF8".to_owned()));
        assert_eq!(
            token(b" UTF8"),
            Error(Err::Position(ErrorKind::Many1, &b" UTF8"[..]))
        );
        assert_eq!(
            token(b"\"UTF8\""),
            Error(Err::Position(ErrorKind::Many1, &b"\"UTF8\""[..]))
        ...
        assert_eq!(
            token(b"\\x41"),
            Error(Err::Position(ErrorKind::Many1, &b"\\x41"[..]))
        );
        assert_eq!(
            token(b"\\101"),
            Error(Err::Position(ErrorKind::Many1, &b"\\101"[..]))
        );
    ...
    fn test_digits() {
        ...
        assert_eq!(
            quad_digit(b"4"),
            Error(Err::Position(ErrorKind::OneOf, &b"4"[..]))
        );
        assert_eq!(
            octal_digit(b"8"),
            Error(Err::Position(ErrorKind::OneOf, &b"8"[..]))
        );
        assert_eq!(
            hex_digit(b"g"),
            Error(Err::Position(ErrorKind::OneOf, &b"g"[..]))
        );
        assert_eq!(
            hex_digit(b"G"),
            Error(Err::Position(ErrorKind::OneOf, &b"G"[..]))
        ...
        assert_eq!(
            hex_digit(b":"),
            Error(Err::Position(ErrorKind::OneOf, &b":"[..]))
        );
        assert_eq!(
            hex_digit(b"@"),
            Error(Err::Position(ErrorKind::OneOf, &b"@"[..]))
    ...
    fn test_octal_esc() {
        ...
        assert_eq!(
            octal_esc(b"\\080"),
            Error(Err::Position(ErrorKind::OneOf, &b"80"[..]))
        );
        assert_eq!(
            octal_esc(b"\\400"),
            Error(Err::Position(ErrorKind::OneOf, &b"400"[..]))
        );

        assert_eq!(
            octal_esc(b"\\x00"),
            Error(Err::Position(ErrorKind::OneOf, &b"x00"[..]))
        );
        assert_eq!(
            octal_esc(b"A"),
            Error(Err::Position(ErrorKind::Tag, &b"A"[..]))
        );
        assert_eq!(
            octal_esc(b" \\000"),
            Error(Err::Position(ErrorKind::Tag, &b" \\000"[..]))
        );
    }
    ...
    fn test_hex_esc() {
        ...
        assert_eq!(
            hex_esc(b"0x41"),
            Error(Err::Position(ErrorKind::Tag, &b"0x41"[..]))
        );
        assert_eq!(
            hex_esc(b"\\000"),
            Error(Err::Position(ErrorKind::Tag, &b"\\000"[..]))
    ...
    fn test_quoted_string() {
        assert_eq!(
            quoted_string(b"\"Stuff\""),
            Done(&b""[..], "Stuff".to_owned())
        );
        assert_eq!(
            quoted_string("\"Stũff\"".as_bytes()),
            Done(&b""[..], "Stũff".to_owned())
        );
        assert_eq!(
            quoted_string(b"\"a\"\"b\""),
            Done(&b"\"b\""[..], "a".to_owned())
        ...
        assert_eq!(
            quoted_string(b"\"A\\x42\""),
            Done(&b""[..], "AB".to_owned())
        );

        assert_eq!(
            quoted_string(b"UTF8"),
            Error(Err::Position(ErrorKind::Tag, &b"UTF8"[..]))
        );

        assert_eq!(
            quoted_string(b"\"A\\x82\""),
            Error(Err::Position(ErrorKind::MapRes, &b"A\\x82\""[..]))
        );
        assert_eq!(
            quoted_string(b"\"A\\xce\""),
            Error(Err::Position(ErrorKind::MapRes, &b"A\\xce\""[..]))
        );
        assert_eq!(
            quoted_string(b"\"A\\xce\\xbb\""),
            Done(&b""[..], "Aλ".to_owned())
        );
        ...
        assert_eq!(
            quoted_string(b"\"A\\102\""),
            Error(Err::Position(ErrorKind::Tag, &b"\\102\""[..]))
        );

        assert_eq!(
            quoted_string(b"\"AC Ra\\xC3\\xADz\""),
            Done(&b""[..], "AC Raíz".to_owned())
        );
        assert_eq!(
            quoted_string("\"Főtanúsítvány\"".as_bytes()),
    ...
    fn test_multiline_octal() {
        ...
        assert_eq!(
            multiline_octal(b"\\101\n\\033"),
            Done(&b""[..], vec![65, 27])
        );
        assert_eq!(
            multiline_octal(b"\\101\\033\n"),
        ...
        assert_eq!(
            multiline_octal(b"\\101 # Sixty-five \n\t\\033"),
            Done(&b""[..], vec![65, 27])
        );
        assert_eq!(
            multiline_octal(b"\\101\\033\nEND"),
    ...
    fn test_token_value() {
        assert_eq!(
            type_and_value(b"CK_BBOOL CK_TRUE\n"),
            Done(
                &b""[..],
        ...
        assert_eq!(
            type_and_value(b"CK_BBOOL CK_TRUE # Very true. Wow. \n"),
            Done(
                &b""[..],
                Value::Token("CK_BBOOL".to_owned(), "CK_TRUE".to_owned())
            )
        ...
        assert_eq!(
            type_and_value(b"MULTILINE_OCTAL CK_TRUE\n"),
    ...
    fn test_string_value() {
        assert_eq!(
            type_and_value(b"UTF8 \"0\"\n"),
            Done(&b""[..], Value::String("0".to_owned()))
        );
        assert_eq!(
            type_and_value(b"UTF8 \"Bogus Mozilla Addons\"\n"),
        ...
        assert_eq!(
            type_and_value(b"UTF8   "),
            Incomplete(Needed::Size("UTF8   ".len() + 1))
        );
        assert_eq!(
            type_and_value(b"UTF8 \""),
            Incomplete(Needed::Size("UTF8 \"".len() + 1))
        ...
        assert_eq!(
            type_and_value(b"UTF8 \"x\""),
            Incomplete(Needed::Size("UTF8 \"x\"".len() + 1))
        );
    }
    ...
    fn test_attr() {
        assert_eq!(
            ...
            Done(
                ...
                (
                    "CKA_CLASS".to_owned(),
                    Value::Token("CK_OBJECT_CLASS".to_owned(), "CKO_CERTIFICATE".to_owned())
                )
            )
        );
        ...
        assert_eq!(
            attribute(
                b"CKA_SERIAL_NUMBER MULTILINE_OCTAL\n\
